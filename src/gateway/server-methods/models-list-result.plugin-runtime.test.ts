...
describe("models.list plugin metadata handoff", () => {
  ...
  it.each([
    ...
    {
      name: "keeps native catalog rows when the ambient registration is retired",
      emptyBase: true,
      supersedeDuringDiscovery: false,
      expectedAvailable: true,
    },
  ])("$name", async ({ supersedeDuringDiscovery, expectedAvailable, emptyBase }) => {
    ...
    await withOpenClawTestState(
      ...
      async (state) => {
        ...
        const snapshot: ModelCatalogSnapshot = {
          entries: emptyBase ? [] : [entry],
          routeVariants: emptyBase ? [] : [entry],
        };
        ...
        const loadActiveCatalog = vi.fn(async () => (emptyBase ? [] : [entry]));
        ...
        try {
          ...
          const view = emptyBase ? "all" : "configured";
          await handler({
            req: {
              type: "req",
              id: "prepared-registry-models-list",
              method: "models.list",
              params: { agentId: "main", view },
            },
            params: { agentId: "main", view },
            respond,
            client: null,
