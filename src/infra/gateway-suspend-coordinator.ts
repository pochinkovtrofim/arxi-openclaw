...
function inspectWakeRequirement(
  inspect: (() => CronSuspendWakeSnapshot) | undefined,
): GatewaySuspendWakeRequirement | undefined {
  const snapshot = inspect?.() ?? { complete: true, nextWakeAtMs: null };
  if (!snapshot.complete) {
    return undefined;
  }
  return snapshot.nextWakeAtMs === null
    ? { kind: "external-event-only" }
    : { kind: "at", atMs: snapshot.nextWakeAtMs };
}
...
function refreshHeldSuspension(held: HeldGatewaySuspension): HeldGatewaySuspension["phase"] {
  ...
  const wakeRequirement = inspectWakeRequirement(held.phase.inspectWakeRequirement);
  if (!wakeRequirement) {
    held.phase.snapshot = snapshot;
    return held.phase;
  }
  ...
  held.phase = { status: "ready", snapshot, wakeRequirement };
  return held.phase;
}
...
function heldPrepareResult(
  ...
  return phase.status === "draining"
    ...
    : { status: "ready", ...result, wakeRequirement: phase.wakeRequirement };
}
...
export function prepareGatewaySuspend(params: {
  ...
  try {
    ...
    const wakeRequirement = snapshot.idle
      ? inspectWakeRequirement(params.inspectWakeRequirement)
      : undefined;
    if (snapshot.idle && !wakeRequirement) {
      const resumed = resumeSchedulingBeforeReopen({
        owner,
        resumeScheduling: params.resumeScheduling,
        reopenAdmission: admission.rollback,
    ...
    const held = armExpiry({
      ...
      expiresAtMs,
      phase: snapshot.idle
        ? { status: "ready", snapshot, wakeRequirement: wakeRequirement! }
        : {
            status: "draining",
            snapshot,
...
export function getGatewaySuspendStatus(suspensionId: string): GatewaySuspendStatusResult {
  ...
  return {
    status: "ready",
    expiresAtMs: held.expiresAtMs,
    wakeRequirement: phase.wakeRequirement,
  };
}
