...
describe("gateway suspend coordinator", () => {
  ...
  it("captures the wake requirement only after admitted drain work settles", () => {
    let queued = 1;
    let nextWakeAtMs = 1_800_000_000_000;
    const inspectWakeRequirement = vi.fn(() => ({
      complete: true as const,
      nextWakeAtMs,
    }));
    const prepared = prepareGatewaySuspend({
      requestId: "request-drain-wake",
      drain: true,
      pauseScheduling: vi.fn(),
      resumeScheduling: vi.fn(),
      inspect: inspectors({ getQueueSize: () => queued }),
      inspectWakeRequirement,
      createSuspensionId: () => "suspension-drain-wake",
    });

    expect(prepared).toMatchObject({ status: "draining" });
    expect(inspectWakeRequirement).not.toHaveBeenCalled();

    nextWakeAtMs = 1_900_000_000_000;
    queued = 0;
    expect(getGatewaySuspendStatus("suspension-drain-wake")).toEqual({
      status: "ready",
