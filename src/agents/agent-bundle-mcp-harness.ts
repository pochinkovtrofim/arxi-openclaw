...
export async function materializeRequesterScopedMcpToolsForHarnessRunCore(
  ...
  try {
    ...
    const applyApprovalPolicy = (candidates: AnyAgentTool[]) => {
      const filtered = applyHarnessToolPolicy(candidates, policyParams);
      return params.scheduledCodexApproval || params.requestInteractiveCodexApproval
        ? applyConfiguredMcpApproval(filtered, {
            fullPermission:
            ...
            ...(params.requestInteractiveCodexApproval
              ? { requestApproval: params.requestInteractiveCodexApproval }
              : {}),
            onOmitted: (message) => scheduledWarnings.push(message),
          })
        : filtered;
    };
    const executableCandidates = params.scheduledCodexApproval
      ...
      : tools;
    const filteredTools = applyApprovalPolicy(executableCandidates);
    const policyAdvertised = applyHarnessToolPolicy(advertisedTools, policyParams);
