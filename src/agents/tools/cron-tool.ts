  ...
  const tool: AnyAgentTool = {
    ...
    execute: async (_toolCallId, args, operationSignal) => {
      ...
      const withCreatorAuthorityProvenance = async <T>(
        authority:
          | (Pick<CronCreatorToolAuthoritySnapshot, "tools" | "provenance"> & {
        ...
        return await withGatewayToolCallerIdentity(
          {
            ...
            ...(authority.grant ? { cronCreatorAuthorityGrant: authority.grant } : {}),
            cronMcpToolBindings: authority.toolsAllow
              ? resolveCronCreatorMcpToolBindingsForToolsAllow(
                  authority.tools,
                  authority.toolsAllow,
                )
              : resolveCronCreatorMcpToolBindings(authority.tools),
          },
          run,
        );
      };
