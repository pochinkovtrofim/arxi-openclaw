...
import { recordTaskFlowHistoryInDatabase } from "./task-flow-history.store.sqlite.js";
...
export function upsertTaskFlowRowInDatabase(
  db: DatabaseSync,
  row: BoundTaskFlowRecord,
  ...
  // Keep receipts and terminal wake cleanup in the same transaction for both worker and sync writers.
  if (
    history ||
    tableExists(db, "task_flow_history_streams") ||
    tableExists(db, "task_flow_automation_obligations")
  ) {
    const flow = readTaskFlowRecord(db, row.flow_id);
    if (!flow) {
      throw new Error("Task Flow upsert did not persist its row.");
    }
    recordTaskFlowHistoryInDatabase(db, flow, history);
    if (
