...
const DOCUMENT_EXTRACTION_FAILURES = new Set<DocumentExtractionFailure>(
  Object.keys(DOCUMENT_FAILURE_MESSAGES) as DocumentExtractionFailure[],
);
...
/** Reads content-free typed failures across separately loaded plugin module graphs. */
export function documentExtractionFailureFromError(
  error: unknown,
): DocumentExtractionFailure | undefined {
  if (!(error instanceof Error) || error.name !== "DocumentExtractionError") {
    return undefined;
  }
  const code = (error as Error & { code?: unknown }).code;
  return typeof code === "string" &&
    DOCUMENT_EXTRACTION_FAILURES.has(code as DocumentExtractionFailure)
    ? (code as DocumentExtractionFailure)
    : undefined;
}
