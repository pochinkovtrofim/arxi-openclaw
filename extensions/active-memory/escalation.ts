...
const LOCALIZED_RECALL_INTENT_PATTERNS = [
  {
    // JS word boundaries are ASCII-only; use Unicode letter boundaries for Russian.
    intent:
      /(?<![\p{L}\p{N}_])(?:помнишь|помните|помнишь ли|вспомни|вспомните)(?![\p{L}\p{N}_])|(?:что|как|где|когда|на ч[её]м)\s+(?:мы|я|ты)\s+(?:обсуждали|решили|договорились|остановились|говорили|выбрали)|какие\s+(?:у меня|мои)\s+предпочтения|напомни(?:те)?[\s,]+(?:что|как|где|когда|на ч[её]м)/iu,
    future:
      /(?<![\p{L}\p{N}_])(?:завтра|послезавтра|сегодня вечером|через\s+\d+|на следующей|в следующий)(?![\p{L}\p{N}_])/iu,
    retrospective:
      /(?:мы|я|ты)\s+(?:обсуждали|решили|договорились|остановились|говорили|выбрали)|(?:вчера|в прошлый раз|раньше)/iu,
  },
  {
    intent:
