...
type DiagnosticsOtelState = {
  spanBindingEmitter?: DiagnosticSpanBindingEmitter | null;
...
export function createDiagnosticsOtelService(): OpenClawPluginService {
  ...
  const stopStarted = async (options?: { preserveExporterRoutes?: boolean }) => {
    ...
    failures.push(
      ...(await settle(
        current.spanBindingEmitter ? () => current.spanBindingEmitter!.retire() : null,
      )),
    );
