...
- **User session override**: `/model`, the model picker, `session_status(model=...)`, and `sessions.patch` write `modelOverrideSource: "user"`. This is an exact session selection by default. If the selected provider/model fails before producing a reply, OpenClaw reports the failure instead of answering from an unrelated configured fallback. Set `agents.defaults.userModelFallbacks: true` to opt explicit user selections into the configured fallback chain while keeping the selected model unchanged for the next turn.
